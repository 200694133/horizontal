...
public final class CacheControl {
     ...
     */
    public static CacheControl parse(HttpHeader headerValue) {
        ...
        String cacheControl = headerValue.value(Headers.CACHE_CONTROL);
        String pragma = headerValue.value(Headers.PRAGMA);
        if (!ValueUtil.isEmpty(cacheControl) || !ValueUtil.isEmpty(pragma)) {
            // Might specify additional cache-control params. We invalidate just in case.
            canUseHeaderValue = false;
        }
        int pos = 0;
        while (pos < cacheControl.length()) {
            int tokenStart = pos;
            pos = HttpUtil.skipUntil(cacheControl, pos, "=,;");
            String directive = cacheControl.substring(tokenStart, pos).trim();
            String parameter;

            if (pos == cacheControl.length() || cacheControl.charAt(pos) == ',' || cacheControl.charAt(pos) == ';') {
                pos++; // consume ',' or ';' (if necessary)
                parameter = null;
            } else {
                pos++; // consume '='
                pos = HttpUtil.skipWhitespace(cacheControl, pos);

                // quoted string
                if (pos < cacheControl.length() && cacheControl.charAt(pos) == '\"') {
                    pos++; // consume '"' open quote
                    int parameterStart = pos;
                    pos = HttpUtil.skipUntil(cacheControl, pos, "\"");
                    parameter = cacheControl.substring(parameterStart, pos);
                    ...
                    int parameterStart = pos;
                    pos = HttpUtil.skipUntil(cacheControl, pos, ",;");
                    parameter = cacheControl.substring(parameterStart, pos).trim();
                }
            }
            ...
            if ("no-cache".equalsIgnoreCase(directive)) {
                ...
                maxAgeSeconds = HttpUtil.parseSeconds(parameter, -1);
            } else if ("s-maxage".equalsIgnoreCase(directive)) {
                sMaxAgeSeconds = HttpUtil.parseSeconds(parameter, -1);
                ...
                maxStaleSeconds = HttpUtil.parseSeconds(parameter, Integer.MAX_VALUE);
            } else if ("min-fresh".equalsIgnoreCase(directive)) {
                minFreshSeconds = HttpUtil.parseSeconds(parameter, -1);
        ...
        return new CacheControl(noCache, noStore, maxAgeSeconds, sMaxAgeSeconds, isPrivate, isPublic,
                mustRevalidate, maxStaleSeconds, minFreshSeconds, onlyIfCached, noTransform, cacheControl);
    }
